...
var baselineDir = filepath.Join("..", "..", "..", "testdata", "parity-baselines", "sharpdpapi")

// TestSharpDPAPIParity validates wasmforge SharpDPAPI against native
// SharpDPAPI baselines captured from Flangvik/SharpCollection.
//
// Lab prerequisites: scripts/lab-setup/sharpdpapi-plant.ps1 must have
// run on win11-ssh as Administrator (drops a DPAPI blob, RDG file,
// imported cert with private key, and cmdkey credential entries).
func TestSharpDPAPIParity(t *testing.T) {
	labctl.SkipIfLabDown(t)

	binary := os.Getenv("WASMFORGE_TEST_BINARY")
	if binary == "" {
		binary = "/tmp/wf-out/sharpdpapi.exe"
	}
