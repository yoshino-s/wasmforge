...
// Opcode classification lookup tables. These use named constants so they remain
// correct regardless of the numeric values assigned to each opcode (supporting
// opcode permutation for anti-analysis builds).
var (
	isLoadStoreOp      [256]bool
	isMemorySizeGrowOp [256]bool
	isConstOp          [256]bool
	isLocalGlobalOp    [256]bool
	isNumericOp        [256]bool
	isRefOp            [256]bool
)

func init() {
	for _, op := range []Opcode{
		OpcodeI32Load, OpcodeI64Load, OpcodeF32Load, OpcodeF64Load,
		...
		isLoadStoreOp[op] = true
	}
	for _, op := range []Opcode{OpcodeMemorySize, OpcodeMemoryGrow} {
		isMemorySizeGrowOp[op] = true
	}
	for _, op := range []Opcode{OpcodeI32Const, OpcodeI64Const, OpcodeF32Const, OpcodeF64Const} {
		isConstOp[op] = true
	}
	for _, op := range []Opcode{OpcodeLocalGet, OpcodeLocalSet, OpcodeLocalTee, OpcodeGlobalGet, OpcodeGlobalSet} {
		isLocalGlobalOp[op] = true
	}
	for _, op := range []Opcode{
		...
		OpcodeI32WrapI64, OpcodeI32TruncF32S, OpcodeI32TruncF32U,
		OpcodeI32TruncF64S, OpcodeI32TruncF64U,
	...
	for _, op := range []Opcode{OpcodeRefNull, OpcodeRefIsNull, OpcodeRefFunc} {
		isRefOp[op] = true
	}
}
