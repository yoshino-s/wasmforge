...
type (
	...
	executionContext struct {
		...
		// memoryFaultTrampolineAddress holds the address of the memory fault handler trampoline function.
		memoryFaultTrampolineAddress *byte
		// memoryFaultAddr holds the base address of the faulting memory access (set by JIT before exit).
		memoryFaultAddr uint64
		// memoryFaultSize holds the access size (offset+opSize) of the faulting memory access (set by JIT before exit).
		memoryFaultSize uint64
	}
)
...
func (c *callEngine) callWithStack(ctx context.Context, paramResultStack []uint64) (err error) {
	...
	for {
		...
		case wazevoapi.ExitCodeMemoryFault:
			faultAddr := c.execCtx.memoryFaultAddr
			faultSize := c.execCtx.memoryFaultSize
			handler, _ := ctx.Value(expctxkeys.MemoryFaultHandlerKey{}).(func(mod api.Module, addr uint32, size uint32) bool)
			if handler != nil {
				mod := c.callerModuleInstance()
				if handler(mod, uint32(faultAddr), uint32(faultSize)) {
					c.execCtx.exitCode = wazevoapi.ExitCodeOK
					afterGoFunctionCallEntrypoint(c.execCtx.goCallReturnAddress, c.execCtxPtr,
						uintptr(unsafe.Pointer(c.execCtx.stackPointerBeforeGoCall)), c.execCtx.framePointerBeforeGoCall)
					continue
