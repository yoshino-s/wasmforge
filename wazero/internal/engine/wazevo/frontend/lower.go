...
func (c *Compiler) memOpSetup(baseAddr ssa.Value, constOffset, operationSizeInBytes uint64) (address ssa.Value) {
	...
	if c.MemoryFaultEnabled {
		// Use AsCallIndirect pattern (like memory.grow) instead of ExitIfTrueWithCode.
		// ExitIfTrueWithCode is a one-way exit: goCallReturnAddress points inside the
		// exit path, and callee-saved registers are not saved. When the Go handler tries
		// to resume JIT via afterGoFunctionCallEntrypoint, it re-enters the exit path,
		// runs exitSequence, returns to callWithStack with ExitCodeOK, and the module
		// terminates prematurely. AsCallIndirect goes through a trampoline that saves
		// all callee-saved registers and sets goCallReturnAddress to the continuation
		// AFTER the call, enabling correct resume.

		faultBlk := builder.AllocateBasicBlock()
		normalBlk := builder.AllocateBasicBlock()

		// Branch: skip handler if in-bounds (cmp == 0 means memLen >= baseAddrPlusCeil).
		brz := builder.AllocateInstruction()
		brz.AsBrz(cmp.Return(), ssa.ValuesNil, normalBlk)
		builder.InsertInstruction(brz)

		// OOB: jump to fault handler block.
		jmpFault := builder.AllocateInstruction()
		jmpFault.AsJump(ssa.ValuesNil, faultBlk)
		builder.InsertInstruction(jmpFault)

		// --- Fault block: call the memory fault trampoline ---
		builder.Seal(faultBlk) // One predecessor: current block.
		builder.SetCurrentBlock(faultBlk)

		// Store fault address and size into executionContext (only on OOB path).
		storeAddr := builder.AllocateInstruction()
		storeAddr.AsStore(ssa.OpcodeStore, extBaseAddr, c.execCtxPtrValue, wazevoapi.ExecutionContextOffsetMemoryFaultAddr.U32())
		builder.InsertInstruction(storeAddr)
		storeSize := builder.AllocateInstruction()
		storeSize.AsStore(ssa.OpcodeStore, ceilConst.Return(), c.execCtxPtrValue, wazevoapi.ExecutionContextOffsetMemoryFaultSize.U32())
		builder.InsertInstruction(storeSize)

		// Store caller module context (needed by Go handler to find the module).
		c.storeCallerModuleContext()

		// Load the fault trampoline address from executionContext.
		faultTrampolinePtr := builder.AllocateInstruction().
			AsLoad(c.execCtxPtrValue,
				wazevoapi.ExecutionContextOffsetMemoryFaultTrampolineAddress.U32(),
				ssa.TypeI64,
			).Insert(builder).Return()

		// Call the trampoline (saves registers, exits to Go, resumes here after).
		faultArgs := c.allocateVarLengthValues(1, c.execCtxPtrValue)
		builder.AllocateInstruction().
			AsCallIndirect(faultTrampolinePtr, &c.memoryFaultSig, faultArgs).
			Insert(builder)

		// Jump to normal continuation.
		jmpNormal := builder.AllocateInstruction()
		jmpNormal.AsJump(ssa.ValuesNil, normalBlk)
		builder.InsertInstruction(jmpNormal)

		// --- Normal block: both paths merge here ---
		builder.Seal(normalBlk) // Two predecessors: current block (via brz) and faultBlk.
		builder.SetCurrentBlock(normalBlk)

		// Reload memory base and length from opaque buffer. On the fault path,
		// memory may have been grown and reallocated. On the in-bounds path,
		// these reload the same values — two extra loads per memory op.
		c.reloadMemoryBaseLen()
	} else {
