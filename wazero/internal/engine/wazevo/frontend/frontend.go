...
type Compiler struct {
	...
	// MemoryFaultEnabled enables the memory fault handler path in bounds checks.
	// When true, OOB memory accesses store the fault address/size in the execution
	// context and exit with ExitCodeMemoryFault instead of ExitCodeMemoryOutOfBounds.
	MemoryFaultEnabled bool
...
func (c *Compiler) declareSignatures(listenerOn bool) {
	...
	c.memoryFaultSig = ssa.Signature{
		ID: c.memoryNotifySig.ID + 1,
		// Takes only execution context. Fault addr/size communicated via execCtx fields.
		Params: []ssa.Type{ssa.TypeI64},
	}
	c.ssaBuilder.DeclareSignature(&c.memoryFaultSig)
}
