...
type (
	...
	sharedFunctions struct {
		...
		// memoryFaultAddress is the address of the memory fault handler trampoline function.
		memoryFaultAddress *byte
...
func (e *engine) compileModule(ctx context.Context, module *wasm.Module, listeners []experimental.FunctionListener, ensureTermination bool) (*compiledModule, error) {
	...
	memoryFaultEnabled := ctx.Value(expctxkeys.MemoryFaultHandlerKey{}) != nil
	...
	if workers := experimental.GetCompilationWorkers(ctx); workers <= 1 {
		...
		fe.MemoryFaultEnabled = memoryFaultEnabled
...
func (e *engine) compileSharedFunctions() {
	...
	// Memory fault handler trampoline. Called via AsCallIndirect when an OOB
	// memory access is detected and MemoryFaultEnabled is true. The trampoline
	// saves/restores callee-saved registers and exits to Go with ExitCodeMemoryFault.
	// The Go handler reads faultAddr/faultSize from executionContext fields
	// (stored by the SSA code before the call) and invokes the user callback.
	e.be.Init()
	addTrampoline(8,
		e.machine.CompileGoFunctionTrampoline(wazevoapi.ExitCodeMemoryFault, &ssa.Signature{
	...
	offset += sizes[7]
	fns.memoryFaultAddress = &fns.executable[offset]
	...
	if wazevoapi.PerfMapEnabled {
		...
		wazevoapi.PerfMap.AddEntry(uintptr(unsafe.Pointer(fns.memoryFaultAddress)), uint64(sizes[8]), "memory_fault_trampoline")
	}
